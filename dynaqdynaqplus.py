...
get_ipython().run_cell_magic('add_to', 'DynaQPlusAgent', '\n# -----------\n# Graded Cell\n# -----------\n    \ndef agent_start(self, state):\n    """The first method called when the experiment starts, called after\n    the environment starts.\n    Args:\n        state (Numpy array): the state from the\n            environment\'s env_start function.\n    Returns:\n        (int) The first action the agent takes.\n    """\n    \n    # given the state, select the action using self.choose_action_egreedy(), \n    # and save current state and action (~2 lines)\n    ### self.past_state = ?\n    ### self.past_action = ?\n    # Note that the last-visit counts are not updated here.\n    \n    # ----------------\n    # your code here\n    action = self.choose_action_egreedy(state)\n    self.past_action = action\n    self.past_state = state\n    # ----------------\n    \n    return self.past_action\n\ndef agent_step(self, reward, state):\n    """A step taken by the agent.\n    Args:\n        reward (float): the reward received for taking the last action taken\n        state (Numpy array): the state from the\n            environment\'s step based on where the agent ended up after the\n            last step\n    Returns:\n        (int) The action the agent is taking.\n    """  \n    \n    # Update the last-visited counts (~2 lines)\n    # - XDirect-RL step (1~3 lines)\n    # - XModel Update step (~1 line)\n    # - X`planning_step` (~1 line)\n    # - XAction Selection step (~1 line)\n    # XSave the current state and action before returning the action to be performed. (~2 lines)\n    \n    # ----------------\n    # your code here\n    self.tau += 1\n    self.tau[self.past_state][self.past_action] = 0\n    #----------------\n    self.q_values[self.past_state][self.past_action] += self.step_size*(reward + self.gamma*np.max(self.q_values[state]) - self.q_values[self.past_state][self.past_action])\n    #-------------------\n    self.update_model(self.past_state,self.past_action,state,reward)\n    #------------------\n    self.planning_step()\n    #--------------\n    action = self.choose_action_egreedy(state)\n    # ----------------   \n    self.past_state = state\n    self.past_action = action\n    \n    return self.past_action\n\ndef agent_end(self, reward):\n    """Called when the agent terminates.\n    Args:\n        reward (float): the reward the agent received for entering the\n            terminal state.\n    """\n    # Again, add the same components you added in agent_step to augment Dyna-Q into Dyna-Q+\n    \n    # ----------------\n    # your code here\n    self.tau += 1\n    self.tau[self.past_state][self.past_action] = 0\n    self.q_values[self.past_state][self.past_action] += self.step_size*(reward - self.q_values[self.past_state][self.past_action])\n    self.update_model(self.past_state,self.past_action,-1,reward)\n    self.planning_step()\n    # ----------------')
