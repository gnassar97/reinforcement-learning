...
def get_value(s, weights):
    ...
    x = np.maximum(psi, np.zeros_like(psi))
...
class TDAgent(BaseAgent):
    ...
    def agent_init(self, agent_info={}):
        ...
        for i in range(self.num_hidden_layer+1):
            ...
            # ----------------
            # your code here
            self.weights[i]["W"] = self.rand_generator.normal(loc = 0, scale = np.sqrt(2/self.layer_size[i]), size = (self.layer_size[i], self.layer_size[i+1]))
            self.weights[i]["b"] = self.rand_generator.normal(loc = 0, scale = np.sqrt(2/self.layer_size[i]), size = (1, self.layer_size[i+1]))
