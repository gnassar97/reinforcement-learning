ssignment 2 - Implement your agent
...
In this assignment, you will use the Adam algorithm for updating the weights of your action-value network. As you may remember from Course 3 Assignment 2, the Adam algorithm is a more advanced variant of stochastic gradient descent (SGD). The Adam algorithm improves the SGD update with two concepts: adaptive vector stepsizes and momentum. It keeps running estimates of the mean and second moment of the updates, denoted by m and v respectively:

mt=βmmt−1+(1−βm)gtvt=βvvt−1+(1−βv)g2t
Here, βm and βv are fixed parameters controlling the linear combinations above and gt is the update at time t (generally the gradients, but here the TD error times the gradients).

Given that m and v are initialized to zero, they are biased toward zero. To get unbiased estimates of the mean and second moment, Adam defines m^ and v^ as:

m^t=mt1−βtmv^t=vt1−βtv
The weights are then updated as follows:

wt=wt−1+αv^t−−√+ϵm^t
Here, α is the step size parameter and ϵ is another small parameter to keep the denominator from being zero.
...
In this assignment, you will use a softmax policy. One advantage of a softmax policy is that it explores according to the action-values, meaning that an action with a moderate value has a higher chance of getting selected compared to an action with a lower value. Contrast this with an ϵ-greedy policy which does not consider the individual action values when choosing an exploratory action in a state and instead chooses randomly when doing so.

The probability of selecting each action according to the softmax policy is shown below:

Pr(At=a|St=s)=˙eQ(s,a)/τ∑b∈AeQ(s,b)/τ
where τ is the temperature parameter which controls how much the agent focuses on the highest valued actions. The smaller the temperature, the more the agent selects the greedy action. Conversely, when the temperature is high, the agent selects among actions more uniformly random.
...
Pr(At=a|St=s)=˙eQ(s,a)/τ−maxcQ(s,c)/τ∑b∈AeQ(s,b)/τ−maxcQ(s,c)/τ
...
compute the preference, H(a), for taking each action by dividing the action-values by the temperature parameter τ,
...
At time t, we have an action-value function represented as a neural network, say Qt. We want to update our action-value function and get a new one we can use at the next timestep. We will get this Qt+1 using multiple replay steps that each result in an intermediate action-value function Qit+1 where i indexes which replay step we are at.

In each replay step, we sample a batch of experiences from the replay buffer and compute a minibatch Expected-SARSA update. Across these N replay steps, we will use the current “un-updated” action-value network at time t, Qt, for computing the action-values of the next-states. This contrasts using the most recent action-values from the last replay step Qit+1. We make this choice to have targets that are stable across replay steps. Here is the pseudocode for performing the updates:

Qt←action-value network at timestep t (current action-value network)Initialize Q1t+1←QtFor i in [1,...,N] (i.e. N replay steps):s,a,r,t,s′←Sample batch of experiences from experience replay bufferDo Expected Sarsa update with Qt:Qi+1t+1(s,a)←Qit+1(s,a)+α⋅[r+γ(∑bπ(b|s′)Qt(s′,b))−Qit+1(s,a)] making sure to add the γ(∑bπ(b|s′)Qt(s′,b)) for non-terminal transitions only.After N replay steps, we set QNt+1 as Qt+1 and have a new Qt+1for time step t+1 that we will fix in the next set of updates. (1)(2)(3)(4)(5)(6)(7)
...
compute the action-values for the next states using the action-value network Qt,
compute the policy π(b|s′) induced by the action-values Qt (using the softmax function you implemented before),
compute the Expected sarsa targets r+γ(∑bπ(b|s′)Qt(s′,b)),
compute the action-values for the current states using the latest Qt+1, and,
compute the TD-errors with the Expected Sarsa targets.
For the third step above, you can start by computing π(b|s′)Qt(s′,b) followed by summation to get v^π(s′)=(∑bπ(b|s′)Qt(s′,b)). v^π(s′) is an estimate of the value of the next state. Note for terminal next states, v^π(s′)=0. Finally, we add the rewards to the discount times v^π(s′).
