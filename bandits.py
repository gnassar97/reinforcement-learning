...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        step_size2 = 1/(self.arm_count[self.last_action])
        
        self.q_values[self.last_action] = self.q_values[self.last_action] + step_size2 * (reward - self.q_values[self.last_action])
        #self.q_values[]
...
class EpsilonGreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        if rand_num < self.epsilon:
            rand_int = np.random.randint(0,len(self.q_values))
            current_action = rand_int
        else:
            current_action = argmax(self.q_values)
        #raise NotImplementedError()
